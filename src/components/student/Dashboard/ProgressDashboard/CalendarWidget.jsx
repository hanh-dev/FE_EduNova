...
const CalendarWidget = () => {
    const [currentDate, setCurrentDate] = React.useState(new Date('2025-05-06'));
    React.useEffect(() => {
      const checkDate = () => {
        const now = new Date();
        if (now.getDate() !== currentDate.getDate() || now.getMonth() !== currentDate.getMonth() || now.getMonth() !== currentDate.getMonth()) {
          setCurrentDate(now);
        }
      };
    ...
    return (
      <div className="calendar-container">
        <div className="calendar-header">
          ...
          <span>{currentDate.toLocaleString('default', { day: 'numeric', month: 'short', year: 'numeric' })}</span>
