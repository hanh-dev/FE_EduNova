...
//Dieu them
// Lấy thắc mắc theo goalId và teacherId
export const getTagTeacherByGoal = async (goalId, teacherId) => {
  try {
    ...
    if (!response.data.success) {
      throw new Error(response.data.error || "Không tìm thấy thắc mắc");
    }
    return response.data.data; // { id, student_name, message, student_id, teacher_response, created_at, goal_id }
  } catch (error) {
    handleApiError(error);
    throw error;
  }
...
export const sendTeacherResponse = async (tagId, teacherResponse) => {
  try {
    ...
    if (!response.data.success) {
      throw new Error(response.data.error || "Gửi phản hồi thất bại");
    }
